...
// if(!(lengthABSOLUTE<2./3.)) 
 truncNonLeaf=Math.log(Math.pow(lfcCenterCored,-1./lfcCenterCored-1.-lfcCenterCored*lfcCenterCored));
...
var howToBoost =  4.;
 ...
 if(SEVEYEStartAdj>lengthP&&lengthP<skyInterSection&&seventhOUTside)howToBoost =4.25098765432100123456789;
...
var spoke_factorLarge =spoke_factor*grPlusOneOverLeaf;
 ...
 if(seventhOUTside&&lengthP>SEVEYEStartAdj-sevMargin&&lfc>zoom)
 {
    ...
    {
  hyperCore+=SEVEYEpow/Math.sqrt(lfc-1.);//SEVEYEpow*4./3./Math.sqrt(lfc);
  
  if(lfc>81.)hyperCore-=Math.log(2.)*2.;
    hyperCore-=1./3.;
...
let loopSolid = 0.
      ...
      //  var borderGap =9. ;
                       ...
                       borderGap= ((((gr*4.)/truncator)*truncator-((leaf*2.)/truncator)*truncator));
...
if(seventhEYEthree)
    ...
    if(((seventhEYE==0.)||(seventhEYE==3.&&!seventhEYEthree)||lengthP>2./3.)&&(!seventhOUTside||(!(SEVEYEStartAdj+ sevMargin >lengthP)||lengthP< SEVEYEStartAdj)))//1./3. supercharges upper level
...
if (counter==0.&&swapped==1)s=spin(s,Math.PI)
