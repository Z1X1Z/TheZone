...
let  FEEDBACKuniforms, FEEDBACKuniformsFlip,wipeUniforms;
        ...
        function setDAWdependantSize()
        {
             ...
             DAWstar= new Float32Array(dawNODES*3);//Elders take EldersLeg*3*2*2 and that as it stands is always less than numberOfBins
             DAWstarColors= new Float32Array(dawNODES*4);
             if(window.INITIALIZED)
             {
     ...
     {    setDynamicSampler2ds();//normally does nothing
         ...
         executeTouchRegime();
           ...
           if(window.DAW)
           {
               ...
               let dawAMPLITUDE =[1,.25,.1];//testarContinuous;//
               let dawNOTE = [20.,36.,30.]//mustarD;//
               let DAWstarStride=0.;
               ...
               for (var g=0; g<dawNODES; g++) {
                   ...
                   for (var g=0; g<1;//dawNODES;
                        g++)
                       
                   {
                       
                       if(isFinite(dawAMPLITUDE[g])&&dawAMPLITUDE[g]!=0.&&isFinite(dawNOTE[g])&&dawNOTE[g]!=0.){
                           
                           let arm =((dawNOTE[g]+twist+12)*radialWarp)%24/24*pi*2.;
                           const lengtOriginal=dawAMPLITUDE[g];//twice applied
                           ...
                           //vop.setHSL(((-dawNOTE[g]+8*uniforms.brelued.value)*uniforms.brelued.value)%24/24., dawNOTE[g]/lightingScaleStar,dawNOTE[g]/lightingScaleStar);//297 is around the highest heard note
    ...
    if(onO){
            ...
            
        {
            ...
            if(isFinite(testarContinuous[g])&&testarContinuous[g]!=0.&&isFinite(mustarD[g])&&mustarD[g]!=0.){
                ...
                if (widt==0)widt=starshipSize;
                //var widt =starshipSize;
              if(testarContinuous[g]==maxTestar)  console.log(arm+"arm  widt"+widt)
