...
function osmdResize()
        {
          osmd.zoom=Math.min(window.innerHeight,window.innerWidth)/800;
          window.osmd.render();
        }
...
//https://wordpress.org/plugins/opensheetmusicdisplay/
    ...
    //drawingParameters: "compacttight",//turns off title, reduces margins, etc.; breaks osmd.cursor when used online, so don't use
      ...
      drawUpToMeasureNumber: 1.+Math.floor(window.innerWidth/window.innerHeight*1.25)// draw all measures, up to the end of the sample
    });
    osmd
      .load(toLoad)
      .then(
