...
function zoomRoutine(){
    ...
    ZR = setZoomRate();
    if (zoom>=.24)//could work as one (also below) but my phone was partially crashing on continuum clover zoomout
        zoomOutEngage = false;
    if(!isFinite(ZR))ZR=1;
    if(!zoomOutEngage&&zoomRate>0.){
        if ((zoom>zoomCone && totalAMP>zoomOutRatchetThreshold&&(on&&!window.touchMode))||xTouch+yTouch!=0)zoom *=ZR;
        else if(uniforms.MetaCored.value||zoom<.24){
            zoom /= ZR;
    ...
    //.000000000000000000000001
        ...
        function pongRoutine(x,y){
             ...
             let xEdge = widthPX;
             let yEdge = heightPX;
             let paddleStrikePosition=uniforms.pongBallCoords.value.y/uniforms.resolution.value.y-((note*flip+twist/2.)+12*100)%12./12.;
