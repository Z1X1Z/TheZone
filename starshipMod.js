                           ...
                           // for (var na = 0; na<3; na+=1)nudgeAdjust=totalAMP**nudgeAdjust
                               if (   !window.iOS )
                               {
                                  tolerance+=(totalAMP)**(totalAMP/4)*totalAMP+(totalAMP)**((totalAMP)*((2**.5)*3*4))//-totalAMPmodified/fractionOfFrame/2//tolerance+=(totalAMP)**(totalAMP/4)*totalAMP+totalAMP**((totalAMP)*(6*2*(2**.5)))
                                    tolerance +=((-leafPermanent+grPermanent+2**.5-2.)*totalAMPmodified-2.*totalAMP)/fractionOfFrame*(leafPermanent+grPermanent)*2//this line is uncertain, particularly coefficients///tolerance +=((-leafPermanent+grPermanent+2**.5)-(totalAMP/2.+totalAMPmodified))/fractionOfFrame
                               }
