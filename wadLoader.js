    ...
    let x = e.clientX- window.innerWidth/2.;
       let volume= -Math.sqrt(y*y+x*x)/(Math.max(window.innerHeight+correlationForText,window.innerWidth)/2.);
        ...
        let frequency = Math.pow(2.,((initialAngleSound[touchNumber.get(e.identifier)])/pi/2*12+correction)/12.)*220.;
                                 ...
                                 sound.play({env:{attack: .1, release:.02,hold:-1},pitch:frequency,volume:0,label:touchNumber.get(e.identifier)});
                                 sound2.play({env:{attack: .1, release:.02,hold:-1},pitch:frequency*2.,volume:volume,label:touchNumber.get(e.identifier)});
}
...
//console.log(frequency)
                         sound.setPitch({pitch: frequency,label:touchNumber.get(e.identifier)});
                         sound2.setPitch({pitch:2.*frequency,timeConstant:0.0,label:touchNumber.get(e.identifier)});
                         sound.setVolume({volume:volume*(((angleSound-initialAngleSound[touchNumber.get(e.identifier)]))/(2.*pi)),timeConstant:0.,label:touchNumber.get(e.identifier)});
                                                         sound2.setVolume({volume:volume*(1.-((angleSound-initialAngleSound[touchNumber.get(e.identifier)]))/(2.*pi)),timeConstant:0.,label:touchNumber.get(e.identifier)});
...
if (navigator.userAgent.toLowerCase().match(/mobile/i)||(navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1)){
    container.addEventListener('touchstart', function(e)
        ...
        {touchNumber.set(e.touches[o].identifier,o);startSound(e.touches[o]);}
  ...
  container.addEventListener('touchend', function(e){
      for(var o=0; o<e.changedTouches.length; o++)
        {sound.stop(String(e.changedTouches[o].identifier));
          sound2.stop(String(e.changedTouches[o].identifier));}
      
  }
                             , false);

    container.addEventListener('touchcancel', function(e){for(var o=0; o<e.changedTouches.length; o++)
    {sound.stop(String(e.changedTouches[o].identifier));sound2.stop(String(e.changedTouches[o].identifier));}}, false);
