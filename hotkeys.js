...
let leafSHADER = -1.3247179572447460259609088544780973407344;
let grSHADER = 1.6180339887498948482045868343656381177203091798057628621;

const leafPermanent = -1.32471795724474602596090885447809734073440;
const grPermanent=1.6180339887498948482045868343656381177203091798057628621;
...
async function finishLoadingAudioFile(){const bb=await  loadAudioFile ();
                                ...
                                function setSevStart()
                                {
                                   ...
                                   )
                                   {
                                    ...
                                    +zb*2.
                                     ...
                                     if(zoom<1.) uniforms.SEVEYEStart.value-=(zb**(4./3.))**((((1.+zb**(4./3.))**(1.-zb**(4./3.)))**zb**zb));
           ...
           let  zoomINITiaLizer =.75//.05;//    1/2**65.;//1.;//
                         let xINITiaLizer=0;//0.0001;   //1./3.;
                           ...
                           "2.701002244218596767553929329640246633289476331894964651863773469240418051679200"+
                            "11809790797271393042239937861933082143530056622297664050624612123026115657700739181619224900"+
                            "35382988016614302670204404399791159886995876926967144315568559275780322601648935728244125"+
                            "6437617141933805797999725772594568421290592464438653138985826696902468607792726091799574968"+
                            "6602084950145606267035253977795137072735308881985436325069821895083134136814713717707097149167"+
                            "4520403265378103460169750049940522378759223566745027167938881345972594010697877906027816721652"+
                            "9674370044314084272585608670721572842760869583983089574271980414308353399478918528477842209"+
                            "636516049766110134759920718179971183895596457812840533680294810397765585948244925223867282425645"+
                            "244272729391649201140452404203330577171154536195942322512114738820640954821096469043023212009558600"+
                            "447326424539323952116844326600671760589489348575360889478462167388055948387353284212182022426819146880"+
                            "415184166943624018540015609616591741174714603765918937289579386466337043194283699829431531967872862790290"+
                            "311071864533835912668262063915350253290567327244395045082202440351200392292958656765166928259161567893211416986205715263888"+
                            "4672914605702526719142122334503174830287328364986263127729255061473982328671828718418212563671464017346390067169550541110"+
                            "3452107347371355152799769974249739268457774001298095234143450324518458605076735141326091847231608487484088"+
                            "48131254972655449534088376612361221166576309446714565716062999"+
...
duperZoom: {value:1.},
    ...
    baseN:{value: 2.7010022442},//e=2.718281828},2.84657807422=pi*e/3//2.7010022442185967675539293296402466332894763318949646518637734692=(pi*e)^2/3^3
        ...
        mandelCloverFactor:{value:.25},//1.+1./leaf
...
/*
                                             if (   window.iOS )window.rez=window.devicePixelRatio/wingsOfRighteousness2;
                                                   ...
                                                   */
                                                  window.rez= window.devicePixelRatio/Number(piE)
...
window.zoomBoost = 1.;
  ...
  window.videoCanvas = null;
    ...
    window.stylusON=false;
            ...
            window.zoomRate=1.;
            window.radialWarp=1.;
            window.trailSecondsLong = secondsToEdge/movementRate*5.;
   ...
   // else if (event.altKey);
     ...
     crownHead[n].needsUpdate=true;
            ...
            stackPositionAttribute.needsUpdate = true;
                    ...
                    if(!window.INITIALIZED)
                    {
                        document.getElementById("spinner1").checked=false;
                        document.getElementById("spinner2").checked=false;
                        document.getElementById("spinner3").checked=false;
                        document.getElementById("spinner4").checked=false;
                        document.getElementById("spinner5").checked=false;
                    }
                    if(number!="no number")
                        {
                    ...
                    if(!window.INITIALIZED&&!(uniforms.fibge4.value||uniforms.fibge3.value||uniforms.fibge2.value||uniforms.fibge1.value))
                        {
                        uniforms.fibge4.value=true;
                        if(!window.INITIALIZED) document.getElementById("spinner4").checked=true;
                        }
...
*/
                ...
                {if(number!="no number")
                    uniforms.OrthoEvery.value=number;
                        ...
                        uniforms.iso_engine.value=!uniforms.iso_engine.value;
    ...
    else if (event.altKey&&(key=="π"||key=="p"))uniforms.pixelSTAR.value=(uniforms.pixelSTAR.value+1)%4;
